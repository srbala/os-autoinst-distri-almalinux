...
sub prepare_test_packages {
    ...
    assert_script_run "curl -o /etc/yum.repos.d/openqa-testrepo-1.repo https://build.almalinux.org/pulp/content/copr/eabdullin1-openqa-almalinux-9-$(arch)-dr/config.repo";
    # install the test packages from repo1
    assert_script_run 'dnf -y --disablerepo=* --enablerepo=openqa-testrepo-1 install tini-static';
...
sub verify_installed_packages {
    validate_script_output 'rpm -q tini-static', sub { $_ =~ m/^tini-static-0.19.0-5.el9$/ };
